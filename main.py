    ...
    if faceCurrFrame:
        for encodeFace, faceLoc in zip(encodeCurrFrame, faceCurrFrame):
            ...
            if matches[matchIndex]:
                ...
                if counter == 0:
                    imgSmallBackground = cv2.resize(imgBackground, (960, 540))
                    cvzone.putTextRect(imgSmallBackground,"Loading", (275, 400))
                    cv2.imshow('Face Authenticator', imgSmallBackground)
                    cv2.waitKey(1)
                    counter = 1
                    modeType = 1
